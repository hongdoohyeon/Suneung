...
import { getDdayInfo } from './dday.js?v=884ab5bc0bbc611d10d5';
(function () {
  ...
  async function mountNextExam() {
    ...
    if (!banner) return;
    try {
      const res = await fetch('data/calendar.json?v=884ab5bc0bbc611d10d5');
      if (!res.ok) return;
  ...
  async function mountSummary() {
    ...
    try {
      const res = await fetch('data/site-summary.json?v=884ab5bc0bbc611d10d5');
      if (!res.ok) return;
      ...
      if (countEl && Number.isInteger(summary.archiveCount)) countEl.textContent = summary.archiveCount.toLocaleString('ko-KR');
