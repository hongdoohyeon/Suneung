...
import { getDdayInfo } from './dday.js?v=a758c824dc29a7890187';
(function () {
  ...
  async function mountNextExam() {
    ...
    if (!banner) return;
    try {
      const res = await fetch('data/calendar.json?v=a758c824dc29a7890187');
      if (!res.ok) return;
  ...
  async function mountSummary() {
    ...
    try {
      const res = await fetch('data/site-summary.json?v=a758c824dc29a7890187');
      if (!res.ok) return;
