...
import { getDdayInfo } from './dday.js?v=a6b6ab6506349e096537';
(function () {
  ...
  async function mountNextExam() {
    ...
    if (!banner) return;
    try {
      const res = await fetch('data/calendar.json?v=a6b6ab6506349e096537');
      if (!res.ok) return;
  ...
  async function mountSummary() {
    ...
    try {
      const res = await fetch('data/site-summary.json?v=a6b6ab6506349e096537');
      if (!res.ok) return;
