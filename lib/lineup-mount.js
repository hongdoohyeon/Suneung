...
import { buildLineup, loadAdmissionsData } from './lineup.js?v=884ab5bc0bbc611d10d5';

const STATE = { mode: 'all', year: '2025', filters: new Set(['safe','fit','aim','risky']) };
...
function ensureSection() {
  ...
  sec.innerHTML = `
    ...
    <div class="lineup-controls">
      <div class="lineup-modes" role="tablist" aria-label="진로 선택">
        <button class="lineup-mode is-active" data-mode="all"        type="button" role="tab" aria-selected="true" tabindex="0">전체</button>
        <button class="lineup-mode"           data-mode="humanities" type="button" role="tab" aria-selected="false" tabindex="-1">인문·사회</button>
        <button class="lineup-mode"           data-mode="natural"    type="button" role="tab" aria-selected="false" tabindex="-1">자연·공학·의약</button>
      </div>
    ...
    <div class="lineup-filters" id="lineupFilters">
      <button class="lineup-filter is-active" data-line="safe"  type="button" aria-pressed="true"><span class="lineup-dot" style="background:#1a8a3a"></span>안정</button>
      <button class="lineup-filter is-active" data-line="fit"   type="button" aria-pressed="true"><span class="lineup-dot" style="background:#0a6cb4"></span>적정</button>
      <button class="lineup-filter is-active" data-line="aim"   type="button" aria-pressed="true"><span class="lineup-dot" style="background:#c84d1f"></span>소신</button>
      <button class="lineup-filter is-active" data-line="risky" type="button" aria-pressed="true"><span class="lineup-dot" style="background:#888"></span>지원 어려움</button>
    </div>
    ...
    <p class="lineup-disclaimer">
      지원 라인은 참고용입니다. 실제 지원 전 대학별 모집요강을 확인하세요.
    </p>
  `;
  ...
  // 이벤트 위임 — 한 번만 바인딩
  const modeList = sec.querySelector('.lineup-modes');
  const activateMode = btn => {
    modeList.querySelectorAll('.lineup-mode').forEach(b => {
      const on = b === btn;
  ...
  modeList.addEventListener('keydown', e => {
    const tabs = [...modeList.querySelectorAll('.lineup-mode')];
    ...
    if (next) { e.preventDefault(); next.focus(); activateMode(next); }
  });
  sec.querySelector('#lineupFilters').addEventListener('click', e => {
    ...
    const active = STATE.filters.has(k);
...
async function render(entries) {
  ...
  if (filtered.length === 0) {
    const off = ['safe','fit','aim','risky'].filter(k => !STATE.filters.has(k));
    const hint = off.length
      ? `<br/><small>꺼진 필터: ${off.map(k => ({safe:'안정',fit:'적정',aim:'소신',risky:'지원 어려움'}[k])).join('·')} — 켜보세요.</small>`
      : '';
    list.innerHTML = `<div class="lineup-empty">조건에 맞는 학과가 없습니다.${hint}</div>`;
    return;
  }
