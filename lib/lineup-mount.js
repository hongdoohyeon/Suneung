...
import { buildLineup, loadAdmissionsData } from './lineup.js?v=20260508i';
  ...
  sec.innerHTML = `
    <header class="lineup-section__head">
      ...
      <div class="lineup-section__year">
        <select id="lineupYear" aria-label="기준 학년도">
          <option value="2025" selected>2025학년도 정시</option>
          <option value="2024">2024학년도 정시</option>
        </select>
      </div>
    </header>
    ...
    <p class="lineup-note">
      * 환산값 = 영역별 백분위에 학교 반영비율을 적용한 가중평균에 영어·한국사 등급 가/감점을 합한 값.
      학교마다 자체 환산표가 다르므로 실제 입시에는 입학처 공식 자료를 함께 확인하세요.
    </p>
  `;
...
function rowHTML(r) {
  ...
  const adjBits = [];
  if (r.engAdj) adjBits.push(`영어 ${r.engAdj > 0 ? '+' : ''}${r.engAdj.toFixed(1)}`);
  if (r.hsAdj)  adjBits.push(`한국사 ${r.hsAdj > 0 ? '+' : ''}${r.hsAdj.toFixed(1)}`);
  const adj = adjBits.length ? `<span class="lineup-row__adj">${esc(adjBits.join(' · '))}</span>` : '';
  return `
