...
async function load() {
  try {
    const [r1, r2, r3, r4] = await Promise.all([
      ...
      fetch(`data/admissions/manual-results.json?v=${DATA_VERSION}`),
    ]);
    ...
    const manualResultsPayload = r4.ok ? await r4.json() : {};
    MANUAL_RESULTS_META = manualResultsPayload._meta || null;
    MANUAL_RESULTS = manualResultsPayload;
    populateSchools();
  } catch (e) {
...
function renderSchool(slug) {
  ...
  if (manualYears.length) {
    html += '<h3 style="margin:32px 0 8px;font-size:15px">학과별 70%컷 참고자료 <span style="background:#3a6bbf;color:#fff;padding:1px 6px;border-radius:3px;font-size:10px">수집·검수</span></h3>';
    ...
    for (const [yr, units] of manualYears) {
      html += `<div class="adm-track" style="background:#eef5ff;border-color:#a8c4e8"><div class="adm-track__label">${escHtml(yr)}학년도 정시 — 학과별 70%컷</div>`;
