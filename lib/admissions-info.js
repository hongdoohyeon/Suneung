...
function fmtRatios(rs) {
  ...
  const html = Object.entries(rs).map(([k, v]) => `<span>${k} ${v}${suffix}</span>`).join('');
  return `<span style="background:${badgeBg};font-weight:600">${badge}</span>` + html;
}
...
function fmtEng(grades) {
  ...
  for (const [g] of sorted) html += `<th>${g}등급</th>`;
  html += '</tr></thead><tbody><tr>';
  for (const [, v] of sorted) html += `<td>${v}</td>`;
  html += '</tr></tbody></table>';
  return html;
}
...
function renderSchool(slug) {
  ...
  const tier = v.tier ? `<span class="adm-tier">${TIER_LABEL[v.tier] || v.tier}</span>` : '';
  let html = `<div class="adm-meta">
    <strong style="font-size:16px">${v.name}</strong>${tier}
    <span class="adm-confidence adm-confidence--${conf}">${confLabel}</span>
    ${v.admissionUrl ? `<br><a href="${v.admissionUrl}" target="_blank" rel="noopener">입학처</a>` : ''}
    ${v.source ? `<div class="adm-source">출처: ${v.source}</div>` : ''}
  </div>`;
  if (!v.tracks.length) {
    ...
    for (const t of v.tracks) {
      ...
      html += `<div class="adm-track__label">${mark}${(t.label || '(라벨 없음)').replace(/[<>]/g, '')}</div>`;
      ...
      if (t.ratios_note && Object.keys(t.ratios_note).length) {
        ...
        html += `<div class="adm-source" style="color:#666">📝 ${note}</div>`;
      }
      if (t.scoreFormula) html += `<div class="adm-source">산출: ${t.scoreFormula}</div>`;
      if (t.math_pick) html += `<div class="adm-source">수학: ${t.math_pick}</div>`;
      if (t.tamgu_pick) html += `<div class="adm-source">탐구: ${t.tamgu_pick}</div>`;
      if (t.english_grades) {
        html += fmtEng(t.english_grades);
      }
      if (t.page) html += `<div class="adm-source">PDF p.${t.page}</div>`;
      html += '</div>';
    }
  }
  ...
  if (mres) {
    ...
    for (const [yr, units] of Object.entries(mres).sort((a,b)=>b[0].localeCompare(a[0]))) {
      html += `<div class="adm-track" style="background:#eef5ff;border-color:#a8c4e8"><div class="adm-track__label">${yr}학년도 정시 — 학과별 백분위 70%컷</div>`;
      html += '<table class="adm-eng-table" style="font-size:12px"><thead><tr><th>학과</th><th>백분위 70%컷</th><th>비고</th></tr></thead><tbody>';
      for (const u of units) {
        html += `<tr><td style="text-align:left">${(u.unit||'').replace(/[<>]/g,'')}</td><td><strong>${u.pct70}</strong></td><td>${(u.note||'').replace(/[<>]/g,'')}</td></tr>`;
      }
  ...
  if (results) {
    ...
    for (const [yr, ydata] of Object.entries(results).sort((a,b)=>b[0].localeCompare(a[0]))) {
      html += `<div class="adm-track"><div class="adm-track__label">${yr}학년도 정시 — ${ydata.unit_count}개 학과</div>`;
      ...
      for (let i=0; i<num_cols; i++) html += `<th>${headers[i] || `col${i+1}`}</th>`;
      html += '</tr></thead><tbody>';
      for (const u of ydata.units.slice(0, 30)) {
        html += `<tr><td style="text-align:left;max-width:160px">${(u.unit||'').replace(/[<>]/g,'')}</td>`;
        for (let i=0; i<num_cols; i++) html += `<td>${u.numbers[i] ?? '-'}</td>`;
        html += '</tr>';
      }
      html += '</tbody></table></div>';
      if (ydata.unit_count > 30) html += `<div class="adm-source">... ${ydata.unit_count-30}개 더</div>`;
      html += '</div>';
    }
  }
  // 정책 (가산/감점/한국사/필수)
    ...
    for (const cat of categoryOrder) {
      ...
      if (cat === 'hanguksa_table') {
        ...
        for (const it of pols[cat]) {
          ...
          html += `<li>${text} <span class="adm-source">p${it.page}</span></li>`;
        }
        html += '</ul>';
      }
      html += '</div>';
...
function renderCompareList() {
  ...
  if (!compareMode) { el.style.display = 'none'; return; }
  ...
  el.innerHTML = `<strong>비교 학교</strong> (${compareSlugs.length}/3): ` +
    compareSlugs.map(s => `<span style="background:#fff;padding:2px 8px;border-radius:4px;margin:0 4px;cursor:pointer" data-rm="${s}">${LOOKUP[s]?.name || s} ✕</span>`).join('') +
...
function renderCompare() {
  ...
  for (const slug of compareSlugs) {
    const v = LOOKUP[slug];
    if (!v) continue;
    html += `<div class="adm-track"><div class="adm-track__label">${v.name}</div>`;
    const ratioTracks = (v.tracks || []).filter(t => t.ratios);
    if (ratioTracks.length) {
      for (const t of ratioTracks.slice(0, 3)) {
        html += `<div style="font-size:12px;margin:6px 0">${t.label.slice(0, 50)}: ${fmtRatios(t.ratios)}</div>`;
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('compareBtn').addEventListener('click', () => {
    ...
    $('compareBtn').textContent = compareMode ? '단일 모드' : '비교 모드';
    $('compareBtn').style.background = compareMode ? '#a52' : '#2a5';
