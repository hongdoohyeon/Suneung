...
function populateSchools(filter = '') {
  ...
  if (officialOnly.length) {
    ...
    group.label = '어디가 공식 입결만 (반영비율 준비 중)';
...
function renderOfficialUniversity(code) {
  ...
  let html = `<div class="adm-meta">
    ...
    <span class="adm-data-badge adm-data-badge--official">어디가 공식 입결</span>
    <div class="adm-source">대학코드 ${escHtml(code)} · 반영비율 준비 중</div>
  </div>`;
  ...
  if (sourceUrl) html += `<div class="adm-status__links"><a href="${escAttr(sourceUrl)}" target="_blank" rel="noopener">어디가 공식 상세</a> · ${escHtml(OFFICIAL_COVERAGE_META?.collectedAt || '')} 확인</div>`;
  html += '</section>';
  ...
  if (full?.units?.length) {
    ...
    html += '<div class="adm-track adm-track--official"><table class="adm-eng-table" style="font-size:12px"><thead><tr><th>학과</th><th>백분위 70%컷</th><th>비고</th></tr></thead><tbody>';
    ...
    html += OFFICIAL_SOURCE_ERROR
      ? `<div class="adm-data-load adm-data-load--error">공식 상세 로드 실패: ${escHtml(OFFICIAL_SOURCE_ERROR)}</div>`
  ...
  $('schoolSelect').addEventListener('change', e => {
    ...
    if (compareMode) {
      if (e.target.value.startsWith('adiga:')) {
        $('info').innerHTML = '<div class="adm-empty">반영비율 준비 중인 대학은 비교 모드에 추가할 수 없습니다.</div>';
        e.target.value = '';
        return;
      }
