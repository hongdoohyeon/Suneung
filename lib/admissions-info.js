...
async function loadOfficialRatios() {
  ...
  if (!OFFICIAL_RATIO_PROMISE) {
    OFFICIAL_RATIO_PROMISE = fetch(`data/admissions/sources/adiga-regular-ratios-2027.json?v=${DATA_VERSION}`)
      .then(response => {
        if (!response.ok) throw new Error('official ratio source fetch failed');
        return response.json();
      })
      .then(payload => {
        OFFICIAL_RATIO_UNIVERSITIES = payload.universities || {};
...
function populateSchools(filter = '') {
  ...
  if (officialOnly.length) {
    ...
    group.label = '어디가 공식 반영비율·입결 (추가 103곳)';
...
function renderOfficialRatioTable(table, index) {
  let html = `<div class="adm-track adm-track--official"><div class="adm-track__label">어디가 공식 표 ${index + 1}</div>`;
  html += '<div class="adm-table-scroll"><table class="adm-eng-table adm-official-ratio-table"><tbody>';
...
function renderOfficialRatioSection(summary, full) {
  const status = summary.ratioStatus || 'no_selection_criteria';
  const available = status === 'structured_ratio_available' || status === 'ratio_text_available';
  const sourceUrl = safeUrl(summary.ratioSourceUrl);
  const label = status === 'structured_ratio_available'
    ? `공식 표 ${summary.ratioTableCount}개`
  ...
  let description = available
    ? '어디가 2027학년도 수능위주전형 화면에서 영역별 반영비율·가산점 자료를 확인했습니다.'
    : status === 'criteria_text_available'
      ? '어디가 공식 화면에 수능위주전형 미시행 또는 기타 전형 기준이 명시돼 있습니다.'
      : '어디가 2027학년도 수능위주전형 화면에 반영비율 자료가 공개되지 않았습니다.';
  let html = `<section class="adm-status ${available ? 'adm-status--available' : 'adm-status--missing'}" aria-label="2027학년도 공식 정시 반영비율 공개 상태">
    <div class="adm-status__heading">
      <strong>2027학년도 정시 반영비율</strong>
      <span class="adm-data-badge ${available ? 'adm-data-badge--official' : 'adm-data-badge--missing'}">${escHtml(label)}</span>
    </div>
    <p>${escHtml(description)}</p>`;
  if (sourceUrl) html += `<div class="adm-status__links"><a href="${escAttr(sourceUrl)}" target="_blank" rel="noopener">어디가 공식 수능위주전형</a> · ${escHtml(OFFICIAL_COVERAGE_META?.collectedAt || '')} 확인</div>`;
  html += '</section>';

  if (full?.ratioTables?.length) {
    html += '<h3 style="margin:24px 0 8px;font-size:15px">영역별 반영비율·가산점 <span class="adm-data-badge adm-data-badge--official">어디가 공식</span></h3>';
    full.ratioTables.forEach((table, index) => { html += renderOfficialRatioTable(table, index); });
  }
  if (full?.sectionText) {
    html += `<details class="adm-official-text"${status === 'ratio_text_available' ? ' open' : ''}>
      <summary>수능위주전형 공식 원문 텍스트 보기</summary>
...
function renderOfficialRatiosForSchool(slug) {
  ...
  let html = '<h3 style="margin:24px 0 8px;font-size:15px">2027학년도 공식 반영비율 <span class="adm-data-badge adm-data-badge--official">어디가 전수 감사</span></h3>';
  html += '<div class="adm-source" style="margin-bottom:8px">표준화 비교 자료와 별도로, 어디가 원문 표·텍스트를 캠퍼스별로 보존했습니다.</div>';
  for (const campus of school.campuses) {
    ...
    if (full?.ratioTables?.length) {
      ...
      html += `<p>${available ? '공식 텍스트 자료를 확인했습니다.' : '어디가 공식 화면에 반영비율 자료가 공개되지 않았습니다.'}</p>`;
    }
    if (sourceUrl) html += `<div class="adm-status__links"><a href="${escAttr(sourceUrl)}" target="_blank" rel="noopener">어디가 공식 수능위주전형</a></div>`;
    html += '</div></details>';
  }
  return html;
}
