'use strict';
// PDF 미리보기 — pdfjs 셀프호스팅(lib/vendor) lazy-load + viewport 적응형 렌더 + 줌 컨트롤.

import { escHtml, safeUrl } from './dom.js?v=a6b6ab6506349e096537';
...
async function renderPdfPage(pdf, pageNum, dpr, containerWidth, zoom = 1) {
  ...
  // aspectRatio 미지원 브라우저 폴백 — 픽셀 높이 명시
...
export async function renderPdf(url, container, metaEl) {
  ...
  try {
    ...
    let zoomTimer = null;
    async function rerenderAll(generation) {
    ...
    function applyZoom(z) {
      ...
      zoomTimer = setTimeout(() => rerenderAll(generation).catch(() => {}), 120);
    }
    ...
    function onKey(e) {
      ...
      const container2 = document.getElementById('previewQViewer') || container;
      if (!container2.offsetParent) return;
    ...
    container.innerHTML = '';
    first.classList.add('preview__page--ready');
    ...
    if (total > 1) {
      ...
      more.addEventListener('click', async () => {
        ...
        try {
          for (let i = renderedPages.length + 1; i <= total; i++) {
          ...
          more.setAttribute('aria-label', `PDF 나머지 페이지 로드 실패. 다시 시도: ${escHtml(err.message || String(err))}`);
        }
      });
    ...
    container.querySelector('.preview__retry').addEventListener('click', () => renderPdf(url, container, metaEl));
