...
export async function loadAdmissionsData() {
  ...
  try {
    const [ratios, results, supported] = await Promise.all([
      fetch('data/admissions/manual-ratios.json?v=884ab5bc0bbc611d10d5').then(r => r.json()),
      fetch('data/admissions/manual-results.json?v=884ab5bc0bbc611d10d5').then(r => r.json()),
      fetch('data/admissions/supported-universities.json?v=884ab5bc0bbc611d10d5')
        .then(r => r.ok ? r.json() : null)
        .catch(() => null),
    ]);
    const generalSet = supported?.general ? new Set(supported.general) : null;
    _data = { ratios, results, generalSet };
    return _data;
  } catch { return null; }
}
...
export async function buildLineup(entries, opts = {}) {
  ...
  // 의·치·한·약·수의 학과는 화이트리스트 무관하게 전 학교 노출
  const MEDICAL_RE = /의예|의학과|의학부|치의예|치의학|치과의예|한의예|한의학|약학|수의예|수의학/;
  ...
  for (const slug of Object.keys(data.ratios)) {
    ...
    const isGeneral = !data.generalSet || data.generalSet.has(slug);
    ...
    for (const u of yearResults) {
      ...
      // 일반 학교 아니면 의·치·한·약·수의 학과만 통과
      if (!isGeneral && !MEDICAL_RE.test(u.unit)) continue;
