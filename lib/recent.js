...
export async function trustStats() {
  ...
  if (latest.typeGroup === 'education' && latest.examYear && latest.month) {
    ...
    const t = ({ csat: '수능', sept: '9모', june: '6모', prelim: '예비' })[latest.type] || '';
    updateLabel = `${latest.gradeYear} ${t}`.trim();
