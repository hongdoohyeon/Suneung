...
export function gradeDistTable(rawCuts, fullScore, absolute, basis = null) {
  ...
  const caption = absolute
    ...
    : `등급별 원점수 컷${basis === 'academy_reverse_calculated' ? ' · 입시기관 역산값' : ''}${fullScore ? ` · 만점 ${fullScore}점` : ''}`;
...
export function renderGradeDist(exam, allCuts) {
  ...
  if (hint) hint.textContent = `1등급 ${reverseCalculated ? '역산값 ' : '컷 '}${cut.rawCuts[0]}점${cut.absolute ? ' · 절대평가' : ''}`;
