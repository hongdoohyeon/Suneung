...
import { $ } from './dom.js?v=884ab5bc0bbc611d10d5';

const GRADES = [1, 2, 3, 4, 5, 6, 7, 8, 9];

// 등급별 원점수 컷 HTML 표 — rawCuts[i] 는 (i+1)등급 컷.
// 절대평가는 rawCuts 길이가 9 미만일 수 있어 부족분은 '—' 채움.
export function gradeDistTable(rawCuts, fullScore, absolute, basis = null) {
  ...
  const basisLabel = basis === 'academy_reverse_calculated'
    ? ' · 입시기관 역산값'
    : (basis === 'academy_integerized_threshold' ? ' · 입시기관 추정 정수 경계' : '');
  const caption = absolute
    ? `등급별 원점수 컷 · 절대평가${fullScore ? ` · 만점 ${fullScore}점` : ''}`
    : `등급별 원점수 컷${basisLabel}${fullScore ? ` · 만점 ${fullScore}점` : ''}`;
...
export function renderGradeDist(exam, allCuts) {
  ...
  if (hint) hint.textContent = `1등급 ${reverseCalculated ? '역산값 ' : (integerizedThreshold ? '추정 경계 ' : '컷 ')}${cut.rawCuts[0]}점${cut.absolute ? ' · 절대평가' : ''}`;
  body.innerHTML = gradeDistTable(cut.rawCuts, cut.fullScore ?? 100, !!cut.absolute, cut.rawCutBasis ?? null);
  return cut;
}
