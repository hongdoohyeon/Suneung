...
export function gradeDistTable(rawCuts, fullScore, absolute) {
  const head = GRADES.map(g =>
    `<th class="grade-table__h grade-table__h--g${g}">${g}</th>`
  ).join('');
  ...
  const caption = absolute
    ...
    : `등급별 원점수 컷${fullScore ? ` · 만점 ${fullScore}점` : ''}`;
...
export function renderGradeDist(exam, allCuts) {
  ...
  // 학평(education)은 고1/고2/고3 컷이 다르므로 학년(studentGrade) 정확 매칭을 우선하고,
  // 학년별 컷이 없으면 5요소 매칭으로 폴백한다(기존 매칭 회귀 방지). 수능 등은 5요소만.
  const cut = (exam.typeGroup === 'education'
    ? allCuts.find(c => matchBase(c) && (c.studentGrade ?? null) === (exam.studentGrade ?? null))
    : null) || allCuts.find(matchBase);
  ...
  body.innerHTML = gradeDistTable(cut.rawCuts, cut.fullScore ?? 100, !!cut.absolute);
  return cut;
}
