...
// 등급별 컷 HTML 표 — cuts[i] 는 (i+1)등급 컷.
// 절대평가는 rawCuts 길이가 9 미만일 수 있어 부족분은 '—' 채움.
export function gradeDistTable(cuts, fullScore, absolute, scoreType = 'raw') {
  ...
  const label = scoreType === 'standard' ? '표준점수' : '원점수';
  const caption = absolute
    ? `등급별 ${label} 컷 · 절대평가${fullScore ? ` · 만점 ${fullScore}점` : ''}`
    : `등급별 ${label} 컷${fullScore && scoreType === 'raw' ? ` · 만점 ${fullScore}점` : ''}`;

  return `
    <table class="grade-table" role="table" aria-label="등급별 ${label} 컷">
      <thead>
...
export function renderGradeDist(exam, allCuts) {
  ...
  const hasStandard = cut && Array.isArray(cut.standardCuts) && cut.standardCuts.some(v => v != null);
  if (!hasRaw && !hasStandard) {
    if (hint) hint.textContent = '준비 중';
    body.innerHTML = `<p class="exam-card__sub">이 시험의 원점수 등급컷 데이터가 아직 없어요.</p>`;
    return null;
  }

  const cuts = hasRaw ? cut.rawCuts : cut.standardCuts;
  const scoreType = hasRaw ? 'raw' : 'standard';
  if (hint) hint.textContent = `1등급 컷 ${cuts[0]}점${scoreType === 'standard' ? ' · 표준점수' : ''}${cut.absolute ? ' · 절대평가' : ''}`;
  body.innerHTML = gradeDistTable(cuts, cut.fullScore ?? 100, !!cut.absolute, scoreType);
  return cut;
}
