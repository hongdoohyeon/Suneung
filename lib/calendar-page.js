...
import { loadCalendar, eventsInMonth, nearestExam, ddayLabel, eventColor, eventDateLabel } from './calendar.js?v=884ab5bc0bbc611d10d5';
...
function renderGrid() {
  ...
  let html = '';
  for (const w of WEEKDAYS) html += `<div class="cal-h" role="columnheader">${w}</div>`;
  for (let i = 0; i < startOffset; i++) html += '<div class="cal-cell cal-cell--empty" role="gridcell" aria-hidden="true"></div>';
  for (let d = 1; d <= lastDay; d++) {
    ...
    html += `
      <div class="cal-cell ${isToday}" role="gridcell" aria-label="${escHtml(dayLabel)}">
