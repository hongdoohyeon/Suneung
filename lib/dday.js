...
const KNOWN_DATES = {
  ...
  2027: '2026-11-19',  // 2027학년도 수능 (목) — 평가원 시행세부계획
  2028: '2027-11-18',  // 2028학년도 수능 (목) — 교육부 공식 발표
};
...
function nextCsatDate(now = new Date()) {
  ...
  const candidates = [
    KNOWN_DATES[year + 1] ? parseLocalDate(KNOWN_DATES[year + 1]) : thirdThursdayOfNov(year),
    KNOWN_DATES[year + 2] ? parseLocalDate(KNOWN_DATES[year + 2]) : thirdThursdayOfNov(year + 1),
  ];
  // now 이후 첫 후보 (자정 기준)
