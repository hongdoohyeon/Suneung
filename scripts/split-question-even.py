...
for id_, _ in splits:
    ...
    even_name = f"{it['gradeYear']}_{it['type']}_id{id_}_q_even.pdf"
    odd_name  = f"{it['gradeYear']}_{it['type']}_id{id_}_q_odd.pdf"
    qDL_odd   = f"{it.get('gradeYear')}학년도_{it.get('type')}_{it.get('subject')}_{it.get('subSubject') or ''}_문제지(홀수형).pdf".replace('__','_')
    qDL_even  = f"{it.get('gradeYear')}학년도_{it.get('type')}_{it.get('subject')}_{it.get('subSubject') or ''}_문제지(짝수형).pdf".replace('__','_')
    if key in existing_q_keys:
        for ov in existing:
            m = ov.get('match', {})
            if (m.get('gradeYear'), m.get('type'), m.get('subject'), m.get('subSubject')) == key and 'questionUrlEven' in ov:
                ov['questionUrl']         = REL_BASE + odd_name
                ov['questionDownload']    = qDL_odd
                ov['questionUrlEven']     = REL_BASE + even_name
                ov['questionDownloadEven']= qDL_even
                ov['_split_local_odd']    = str(OUT / odd_name)
                ov['_split_local_even']   = str(OUT / even_name)
                ov['_target_odd']         = odd_name
                ov['_target_even']        = even_name
        continue
    existing.append({
        'match': {
        ...
        'questionUrl':         REL_BASE + odd_name,    # 홀수 분리본
        'questionDownload':    qDL_odd,
        'questionUrlEven':     REL_BASE + even_name,
        'questionDownloadEven':qDL_even,
