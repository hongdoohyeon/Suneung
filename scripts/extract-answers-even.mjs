// 답지 PDF 일괄 텍스트 파싱 → data/answers-even.json 갱신
//
// 사용:
...
const OUT_PATH   = path.join(ROOT, 'data', 'answers-even.json');
...
async function main() {
  ...
  let targets = exams.filter(e => e.answerUrlEven);
  ...
  for (const e of targets) {
    if (!byUrl.has(e.answerUrlEven)) byUrl.set(e.answerUrlEven, []);
    byUrl.get(e.answerUrlEven).push(e);
  }
  const totalWithUrl = exams.filter(e => e.answerUrlEven).length;
  console.log(
